...
<img src="adminpannel/Admin/images/<%=rs.getString("imagea")%>" height="300" width="200" alt="">
</span>
</div>
</div>
<div class="col-md-7">
...
<div class="">
    <%  int flag=1;
          String zip=request.getParameter("zip");
            ...
            PreparedStatement ps3=cn.prepareStatement("SELECT * FROM zipcode WHERE zip=?;");
            ...
            if(rs3.next()){
                ...
                if(rs3.getString("zip").equals(zip)){
...
%>
 ...
 <form action="orders?pid=<%=rs.getString("pid")%>&email=<%=emailu%>" method="post">
     ...
     <input type="hidden" name="imagea" value="<%=rs.getString("imagea")%>" >
     <input type="hidden" name="imageb" value="<%=rs.getString("imageb")%>" >
     <input type="hidden" name="cat" value="<%=rs.getString("cat")%>" >
     <input type="hidden" name="subcat" value="<%=rs.getString("subcat")%>" >
...
<span><strong>Category:</strong> <a href="#"><%=rs.getString("subcat")%></a>
    ...
    <form action="zipcodecheck.jsp" method="post">
        <input type="number" disabled="" name="zip" value="<%=zip%>"> 
        <input type="submit" disabled="" class="btn btn-success"></form>
...
<div class="">
    ...
    <div><h>Please enter other zip code</h></div>
    <form action="zipcodecheck.jsp?pid=<%=pid%>&&discount=<%=discount%>&&subcat=<%=rs.getString("subcat")%>" method="post">
        <input type="number" required="" max="9999999" min="100" name="zip" value=""> <input type="submit" class="btn btn-success"></form>
</div><br>
        <div class="alert alert-danger alert-dismissable">
<button type="button" class="close" data-dismiss="alert" aria-hidden="true">
&times;
</button>
Your zip code is not available for delivery.
</div>
                              <%}%>
</div>
</div>
...
<img src="adminpannel/Admin/images/<%=rs1.getString("imagea")%>" height="250" width="170" alt="">
<img src="adminpannel/Admin/images/<%=rs1.getString("imageb")%>" class="rotate-image" alt="">
</div>
</a>
<div class="shop-item-title clearfix">
<h4><a href="shop-single.jsp"><%=rs1.getString("pname")%></a></h4>
